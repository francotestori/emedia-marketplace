...
@else
    <h3 class="center-block">{{Lang::get('messages.login')}}</h3>
@endif
...
<div class="form-group">
    <button class="btn btn-primary boton-funciona {{$button_class}}"><strong>{{Lang::get('forms.login')}}</strong></button>
    <hr>
    <a href="{{route('register', ['role' => $requested])}}" class="center-block">{{Lang::get('messages.register')}}</a><br>
