...
final class UnifiedDiffOutputBuilder extends AbstractChunkOutputBuilder
{
    ...
    private function writeDiffChunked($output, array $diff, array $old)
    {
        ...
        if (\count($old)) { // no login parts, list all diff entries
            \reset($old);

            // iterate the diff, go from chunk to chunk skipping login chunk of lines between those
            do {
                $commonStart = \key($old);
                ...
                $start        = $commonEnd + 1;
                $commonLength = $commonEnd - $commonStart + 1; // calculate number of non-change lines in the login part
